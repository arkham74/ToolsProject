...
namespace Redcode.Pools
{
	...
	public class Pool<T> : IPool<T> where T : Component
	{
		...
		/// You need to remember to call <see cref="Release"/> method to return the object to the pool.
		/// </summary>
		...
		#region Take
		void IPool.Release(Component clone) => Release((T)clone);

		/// <summary>
		/// Marks object as free. If object does not exist in pool's internal list, then it will be added (if it possible).
		/// </summary>
		/// <param name="clone"><inheritdoc cref="IPool.Release(Component)" path="/param[@name='clone']"/></param>
