...
namespace Crosstales.TPB
{
	...
	public static class Builder
	{
      ...
      public static void BuildTPS()
      {
         ...
         {
            bool forceBatchmode = CTEditorPrefs.GetBool(Constants.KEY_BATCHMODE);

            if (CTEditorPrefs.HasKey(Constants.KEY_TARGETS))
            {
               string[] targets = CTEditorPrefs.GetString(Constants.KEY_TARGETS).Split(';');

               if (targets.Length > 0)
               {
                  if (targets.Length == 1)
                  {
                     CTEditorPrefs.SetString(Constants.KEY_TARGETS, string.Empty);
                  }
                  else
                  {
                     ...
                     CTEditorPrefs.SetString(Constants.KEY_TARGETS, result);
                  }
