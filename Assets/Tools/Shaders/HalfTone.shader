Shader "SleeplessOwl/Post-Processing/HalfTone"
{
	Properties
	{
		...
		_Density ("_Density", Range(0,200)) = 0
		_Radius ("_Radius", Range(0,1)) = 0.3
		_SmoothEdge ("_SmoothEdge", Float) = 0.2
		_HalfToneFactor ("_HalfToneFactor", Float) = 0.5
		_SourceFactor ("_SourceFactor", Float) = 0.5
		_Lightness ("_Lightness", Float) = 1

		_Color01 ("_Color01", Color) = (0, 0, 0, 1)
		_ColorFactor ("_ColorFactor", Color) = (1, 1, 1, 1)
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				float3 texColor = SAMPLE_TEXTURE2D_X(_MainTex, sampler_MainTex, i.uv.xy).rgb;
